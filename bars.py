...
def get_distance_to_bar(point_lon, point_lat, bar_lon, bar_lat):
    distance_to_bar = sqrt((point_lon - bar_lon) ** 2 + (point_lat - bar_lat) ** 2)
...
if __name__ == '__main__':
    ...
    biggest_bar_name = get_biggest_bar(bars_data)['properties']['Attributes']['Name']
    print("Самый большой бар: {}".format(biggest_bar_name))
    smallest_bar_name = get_smallest_bar(bars_data)['properties']['Attributes']['Name']
    print("Самый маленький бар: {}".format(smallest_bar_name))
    nearest_bar_name = get_closest_bar(bars_data, user_lon, user_lat)['properties']['Attributes']['Name']
